...
/*討論與心得:寫完這支程式後，我突然意識到它就像一位細心的老師，一邊指引使用者輸入正確的資料，一邊不斷提醒「這裡要注意範圍」「這裡要六位數」，好像在陪伴一個還不太懂規則的小朋友慢慢長大。比較糟糕的時候，當我把學號當數字印出來，亂碼一堆跳出來，就像老師口誤導致學生滿頭霧水，讓我明白細節有多重要。

每次重複寫同樣的檢查迴圈，手指敲到發痛，心裡就想：要是有個小助手專門幫我驗證就好了。於是我決定把共用的輸入檢查抽成一個小函式，未來只要叫它一次，就能完成所有科目的驗證，程式會變得更簡潔不亂，也減少出錯機會。

總之，這次練習讓我體會到：程式不只是寫給電腦看的，更是在跟使用者對話，必須貼心地告訴他們該做什麼、不要犯哪些錯，才能讓大家都放心。未來我會繼續打磨這段「人機對話」，讓它既可靠又溫暖。 */
